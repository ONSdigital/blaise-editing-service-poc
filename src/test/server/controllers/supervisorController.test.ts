...
import { CaseEditInformationListMockObject, MappedSupervisorInformationMockObject } from '../../mockObjects/CaseMockObject';
...
describe('Get case edit information tests', () => {
  ...
  it('When given a valid quetsionnaire It should return a 200 response with an expected list of Supervisor details', async () => {
    // arrange
    ...
    // act
    const response: Response = await sut.get(`/api/${questionnaireName}/supervisor/edit`);

    // assert
    expect(response.status).toEqual(200);
    expect(response.body).toEqual(MappedSupervisorInformationMockObject);
    blaiseApiMock.verify((api) => api.getCaseEditInformation(questionnaireName), Times.once());
  });

  it('It should return a 500 response when a call is made to retrieve a list of Supervisor details and the rest api is not availiable', async () => {
    // arrange
  ...
  it('It should return a 404 response when a call is made to retrieve a list of supervisor details and the client returns a 404 not found', async () => {
    // arrange
