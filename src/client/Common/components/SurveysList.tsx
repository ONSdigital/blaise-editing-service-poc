...
function CreateContent(surveys:Survey[]):ExpandableContent[] {
  return surveys.map(({ name, questionnaires }) => ({
    title: name, content: <QuestionnairesList questionnaires={questionnaires} />, contentId: 'survey',
  }));
}
...
export default function SurveysList({ surveys }: SurveysListProps): ReactElement {
  ...
  return (
    <>
      <br />
      <Accordion ContentId="survey" Expandables={CreateContent(surveys)} Expanded />
    </>
  );
}
